...
class Profile(models.Model):  # model that expanse User Model
...
class Brand(models.Model):  # model connected with car brands
    BRANDS = (
        (0, 'Audi'),
        (1, 'BMW'),
        (2, 'Citroen'),
...
class Type(models.Model):   # model connected with car types
    TYPES = (
        (0, 'Convertible'),
        (1, 'Coupe'),
        (2, 'Hatchback'),
...
class Location(models.Model):  # Location Model
...
class Rent(models.Model):  # Model which allows to reserve a car
